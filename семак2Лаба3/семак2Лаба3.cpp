...
unsigned int GetNumberOfTimeMoments(const double x, const double T, const double h);
unsigned int DefineBehavior(const double x, const double T, const double h, std::string Name);
...
int main() 
{
	...
	std::cout << "Начальное условие x0:\n";
	x = GetCorrectEnteredNumber();
	...
	while (true)
	{
		...
		if (t > T)
			...
			std::cout << "Начальное условие больше левой границы интервала! Повторите ввод.\n";
		else
			break;
	}
	...
	std::cout << "Обратите внимание, Вы можете добавить не более трёх решателей на каждый из методов!\n";
	...
	while (true)
	{
		...
		// выбор метода
		while (true) // на каждый метод можно добавить максимум по 3 решателя! Дабы посмотреть разные значения 
		{
			...
			if (CounterEulerMethod > 3 || CounterMethodHoina > 3)
				std::cout << "Ошибка,нельзя выбрать более трёх решателей одним методом. Выберите другой метод решения. \n";
			else
				break;
		...
		while (true)
		{
			...
			try 
			{
				...
				Behavior = DefineBehavior(x, T, h, Name);

				//подсчёт временных моментов, в которых будет посчитано решение
				NumberOfTimeMoments = GetNumberOfTimeMoments(x, T, h);

				//выбор решателя в зависимости от выбора пользователя
				if (SelectedSolver == 1)
					Solver.AddSolverEulerMethod(Name, Behavior, NumberOfTimeMoments);
				else
					Solver.AddSolverMethodHoina(Name, Behavior, NumberOfTimeMoments);
	...
	// подсчёт количества заданных решателей
	int NumberOfExistingSolvers = Solver.GetSumOfCountersMethods();
...
unsigned int GetNumberOfTimeMoments(const double x, const double T, const double h)
{
	unsigned int NumberOfTimeMoments = 0;
	double i = x;
	for (i; i <= T; i += h)
		NumberOfTimeMoments++;
	return NumberOfTimeMoments;
}
	...
	for (double i = x; i <= T; i += h)
		if (((i + h) > T) && (i != T))
			while (true)
			{
				...
				Behavior = GetCorrectEnteredNumber();
